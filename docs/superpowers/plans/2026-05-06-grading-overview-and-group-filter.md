...
```bash
sshpass -p '<password — voir TESTING.local.md>' rsync -avz --delete --exclude='.git*' --exclude='*.bak.*' \
    redaction/ \
...
```bash
sshpass -p '<password — voir TESTING.local.md>' ssh favi5410@favi5410.odns.fr "cat > /tmp/p.php <<'PHPEOF'
<?php
...
Compte enseignant `prof / <password — voir TESTING.local.md>` sur le cours TEST (id 2) :
